...
dm_names = open('./traj_files_names.txt','r')

for dm_file_name in dm_names:
    
    print("\n ###########  Lendo arquivo " + dm_file_name.split('.')[0] + "  ###########")
    
    for chunk in md.iterload(dm_file_name.split('.')[0]+'.dcd', chunk=10, top='imaged'+'.pdb'):
        ...
        selection.save_pdb("tmp_frame.pdb")
        
        D = []
        
        with open('tmp_frame.pdb', 'r') as f:
            data = f.readlines()
        ...
        #CALCULO DE MATRIZ DE SIMILARIDADE
        ...
        for i in range(n):
            for j in range(n):
                di = (1/2)*(abs(D) - abs(D - 2*abs(np.array(X[i]) - np.array(X[j]))))
                d = -sum(di**2)
                A[i,j] = d
                
        ap = cluster.AffinityPropagation(damping=0.5, affinity='precomputed').fit(A) #,preference=-1)
#        ap = cluster.AffinityPropagation(damping=0.5).fit(X)
        #ms = cluster.MeanShift(cluster_all=False).fit(X)                
        #db = cluster.DBSCAN(eps=0.135, min_samples=2, algorithm='kd_tree', metric='euclidean').fit(X)
        ...
        plt.savefig("graf_disp_ap/plot_"+str(FRAME)+".png")
        plt.close()
        #plt.show()
...
Data_analysis.to_csv("graf_disp_ap/data_analysis.csv")
